                    ...
                    <div class="card">
                        <div class="content">
                            ...
                            <table id="bootstrap-table" class="table">
                                ...
                                <th data-field="event" class="text-center">Событие</th>
                                <th data-field="event_date" class="text-center">Дата события</th>
                                <th data-field="no_of_tickets" class="text-center">Количество билетов</th>
                                <th data-field="booked_by" class="text-center">Забронировано</th>
                                <th data-field="status" data-sortable="true">Статус</th>
                                <th data-field="payment" data-sortable="true">Платеж</th>
                                        ...
                                        @foreach($event_bookings as $index => $event_booking)
                                            <tr>
                                                ...
                                                <td>{{ $event_booking->user->first_name." ".$event_booking->user->last_name }}<br>
                                                    <strong>почта: </strong>{{ $event_booking->user->email }}
                                                </td>
                                                <td>
                                                    @if($event_booking->status == "1")
                                                        <button class="btn btn-success btn-xs btn-fill">Активный</button>
                                                    @else
                                                        <button class="btn btn-danger btn-xs btn-fill">Отмененный
                                                        </button>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if($event_booking->payment == 1)
                                                        <button class="btn btn-success btn-xs btn-fill">Оплачено</button>
                                                    @else
                                                        <button class="btn btn-default btn-xs btn-fill">Не оплачено
                                                        </button>
                                                    @endif
                                                </td>
                                                <td>
